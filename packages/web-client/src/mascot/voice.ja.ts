...
export const wizdaLinesJa = {
  ...
  errors: {
    ...
    unknownGearKind: `その${JA_CATEGORY}や${JA_RANK}は、まだわたしの手帳にないみたい——絞り込みから外しておいたわ。`,
    generic: "ちょっと私の調子がよくなかったみたい。ごめんね、少し待ってもう一度試してみて。",
  },
  away: {
    title: "新しい情報を探しに王都までお出かけ中！卯数分したらまた覗いてみて！",
    back: "ただいま！帰ってきたわ。",
  },
  about: {
  ...
  confirm: {
    ...
    blessingUnrollableOne: (labels) => `あなたが選んだどれも、${labels}の${JA_BLESSINGS}は引けないわ。`,
    ...
    blessingComboUnrollable: (labels) => TsUtilities.stringJoin([
      `あら！ 選んだどの一枚も、${labels}を同時には持てないの、`,
      `${JA_BLESSINGS}は狙っている${JA_EQUIPMENT}についてこそ意味があるのよ。`,
    ]),
    blessingFloorPhrase: (count, gradeName, atMax) => {
      ...
      return `${subject}なら${target}が必要ね`;
    },
    gradeFloorTooHigh: (floorPhrase) => TsUtilities.stringJoin([
      `${floorPhrase}、でもその${JA_EQUIPMENT}はそこまで高くは出ないみたい！`,
      `${JA_BLESSINGS}を減らすか、別のものを掘るしかないわね。`,
    ]),
    gradeTooHigh: (gradeName) => `うーん……その${JA_EQUIPMENT}は${gradeName}ほど高くは出ないと思うわ。`,
    qualityTooHigh: (qualityLabel) => `選んだ${JA_EQUIPMENT}は${qualityLabel}には届かないみたい！`,
  },
