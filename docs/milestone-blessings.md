...
| **slot was occupied** | the blessing keeps its identity, its value rises | initial roll **+** one increment |
...
- The **increment** is derived from the published tables. See [Enhancement increase](#enhancement-increase).

Two consequences worth carrying into the tool:

1. **A filled slot gets an initial roll and nothing more** — no increment on top. A slot that arrived occupied ends up materially stronger, because only it collects both rolls.
2. **A blessing present at drop is boosted exactly once**, at its own milestone. Its final value is `initial + one increment`, never four.
...
A 4★ Ebonsteel dagger with a flat ATK blessing at **+8**, enhanced to +5. Value group `RANK_1_5`, quality 4, Flat → the increment is uniform over `3-10`, eight possible values. Reaching **≥16** needs an increment of ≥8, so 8, 9 or 10 → **3/8 = 37.5%**.
...
### Deriving the milestone increment

The official page publishes no "enhancement increase" table. It does not need to — the increment falls out of the three it does publish.
...
```
lfas  =  drop ⊛ increment
fas        =  drop ⊛ increment ⊛ increment
increment  =  Uniform[ lfas.min − drop.min , lfas.max − drop.max ]
```

The increment is recovered from the supports alone, then **proved** by reconvolving it and checking it reproduces both published tables cell for cell:

| check | result |
|---|---|
| every `drop` value distribution is uniform | **380/380** |
| `drop ⊛ increment` reproduces `lfas` | **380/380** |
| `drop ⊛ increment ⊛ increment` reproduces `fas` | **380/380** |
...
| **H-increment** | the slot is boosted from zero — the same path as an occupied slot | `Uniform(increment range)` |
| **H-sum** | the slot is filled *and* boosted in one step | `initial ⊛ increment` |

Their supports differ, so a value outside a model's range kills it permanently. Ten White 3★ `SILVER_OTHER` pieces were enhanced to +5 on **2026-08-22** (logged in the [appendix](#observations-log)). The result: **H-sum eliminated twice, H-increment eliminated four times, and all ten values inside H-initial's ranges.**
...
The milestone boost applied to a slot that already held a blessing. Not published; derived per group as `lfas − drop` on the supports and proved by reconvolution, from `<h1>…by Equipment Rank</h1>` and `<h1>…When Using Lesser Full Alteration Stones</h1>`.
...
- Altering **before** that slot's checkpoint: the Alteration Stone's value replaces the blessing, and the later milestone still adds its increment as normal.
...
- There is **no "double boost" for Silver** anywhere in it, despite rumors from players. Silver gets *higher* values, not two applications. The one real doubling is `fas = drop ⊛ increment ⊛ increment`, which holds for every group.
...
2. **Does the empty-slot rule hold outside Silver?** Every observation so far was `SILVER_OTHER`. A 4★ `RANK_1_5` piece rolling a percentage blessing decides it in a single roll (H-increment predicts exactly `3`, H-initial `9-13`).
3. **Relic / collab gear.** The FFXI event guide says Relic items "follow Silver level table ranges" — check whether they land in `SILVER_OTHER` or need their own group.
...
|---|---|---|
| Flat | 2-4 | **H-increment** — settled |
| Flat | 5-6 | narrows to H-increment or H-initial |
| Flat | 7-8 | narrows to H-increment or H-initial or H-sum |
| Flat | 9-10 | narrows to H-increment or H-sum |
| Flat | 11-18 | **H-sum** — settled |
| Percent (%) | 2-4 | **H-increment** — settled |
| Percent (%) | 5-6 | **H-initial** — settled |
| Percent (%) | 7-11 | narrows to H-initial or H-sum |
| Percent (%) | 12-15 | **H-sum** — settled |
| ASPD | 1-3 | **H-increment** — settled |
| ASPD | 4 | narrows to H-increment or H-initial |
| ASPD | 5-7 | narrows to H-initial or H-sum |
| ASPD | 8-11 | **H-sum** — settled |
| ASPD% | 2-3 | **H-increment** — settled |
| ASPD% | 4 | narrows to H-increment or H-initial |
| ASPD% | 5 | **H-initial** — settled |
| ASPD% | 6-7 | narrows to H-initial or H-sum |
| ASPD% | 8-11 | **H-sum** — settled |
| SUR | 1-2 | **H-increment** — settled |
...
| 2026-08-22 | Silver Light Gauntlets | `SILVER_OTHER` | 3★ | ACC% | 7 | H-increment |
...
| 2026-08-22 | Silver Shield | `SILVER_OTHER` | 3★ | DEF% | 11 | H-increment |
| 2026-08-22 | Silver Shield | `SILVER_OTHER` | 3★ | SUR | 5 | H-increment |
| 2026-08-22 | Blackwing Small Shield | `SILVER_OTHER` | 3★ | ASPD | 6 | H-increment |

### Full discriminator table

Every combination, with the values that decide between the three models. `H-sum` is `initial ⊛ increment`, so only a roll above the initial range's top can prove it. `Silver — named pieces` is omitted: every cell is identical to `Worn–Ebonsteel (rank 1-5)`.

| Value group | Blessing | Quality | H-initial | H-increment | H-sum | decisive if you roll |
|---|---|---|---|---|---|---|
| Worn–Ebonsteel (rank 1-5) | Flat | 1★ | 2-3 | 1-2 | 3-5 | 1 ⇒ H-increment; 4-5 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | Flat | 2★ | 3-5 | 2-5 | 5-10 | 2 ⇒ H-increment; 6-10 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | Flat | 3★ | 5-7 | 2-8 | 7-15 | 2-4 ⇒ H-increment; 9-15 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | Flat | 4★ | 8-10 | 3-10 | 11-20 | 3-7 ⇒ H-increment; 11-20 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | Flat | 5★ | 11-13 | 3-12 | 14-25 | 13 ⇒ H-initial; 3-10 ⇒ H-increment; 14-25 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | Percent (%) | 1★ | 1-3 | 1 | 2-4 | 4 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | Percent (%) | 2★ | 3-6 | 1-2 | 4-8 | 3 ⇒ H-initial; 1-2 ⇒ H-increment; 7-8 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | Percent (%) | 3★ | 5-9 | 2-3 | 7-12 | 5-6 ⇒ H-initial; 2-3 ⇒ H-increment; 10-12 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | Percent (%) | 4★ | 9-13 | 3 | 12-16 | 9-11 ⇒ H-initial; 3 ⇒ H-increment; 14-16 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | Percent (%) | 5★ | 12-16 | 3-4 | 15-20 | 12-14 ⇒ H-initial; 3-4 ⇒ H-increment; 17-20 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | ASPD | 1★ | 1-2 | 1-2 | 2-4 | 3-4 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | ASPD | 2★ | 2-4 | 1-2 | 3-6 | 1 ⇒ H-increment; 5-6 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | ASPD | 3★ | 4-6 | 1-3 | 5-9 | 4 ⇒ H-initial; 1-3 ⇒ H-increment; 7-9 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | ASPD | 4★ | 6-8 | 1-4 | 7-12 | 6 ⇒ H-initial; 1-4 ⇒ H-increment; 9-12 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | ASPD | 5★ | 8-10 | 1-5 | 9-15 | 8 ⇒ H-initial; 1-5 ⇒ H-increment; 11-15 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | ASPD% | 1★ | 1-2 | 1 | 2-3 | 3 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | ASPD% | 2★ | 2-4 | 1-2 | 3-6 | 1 ⇒ H-increment; 5-6 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | ASPD% | 3★ | 4-6 | 2-3 | 6-9 | 4-5 ⇒ H-initial; 2-3 ⇒ H-increment; 7-9 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | ASPD% | 4★ | 6-9 | 3 | 9-12 | 6-8 ⇒ H-initial; 3 ⇒ H-increment; 10-12 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | ASPD% | 5★ | 9-11 | 3-4 | 12-15 | 9-11 ⇒ H-initial; 3-4 ⇒ H-increment; 12-15 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | SUR | 1★ | 1-2 | 1 | 2-3 | 3 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | SUR | 2★ | 2-3 | 1-2 | 3-5 | 1 ⇒ H-increment; 4-5 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | SUR | 3★ | 3-4 | 1-2 | 4-6 | 3 ⇒ H-initial; 1-2 ⇒ H-increment; 5-6 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | SUR | 4★ | 4-5 | 1-3 | 5-8 | 4 ⇒ H-initial; 1-3 ⇒ H-increment; 6-8 ⇒ H-sum |
| Worn–Ebonsteel (rank 1-5) | SUR | 5★ | 5-6 | 1-4 | 6-10 | 5 ⇒ H-initial; 1-4 ⇒ H-increment; 7-10 ⇒ H-sum |
| Silver — 2H melee | Flat | 1★ | 2-4 | 1-2 | 3-6 | 1 ⇒ H-increment; 5-6 ⇒ H-sum |
| Silver — 2H melee | Flat | 2★ | 3-7 | 2-7 | 5-14 | 2 ⇒ H-increment; 8-14 ⇒ H-sum |
| Silver — 2H melee | Flat | 3★ | 5-9 | 2-11 | 7-20 | 2-4 ⇒ H-increment; 12-20 ⇒ H-sum |
| Silver — 2H melee | Flat | 4★ | 8-13 | 3-13 | 11-26 | 3-7 ⇒ H-increment; 14-26 ⇒ H-sum |
| Silver — 2H melee | Flat | 5★ | 11-17 | 3-15 | 14-32 | 3-10 ⇒ H-increment; 18-32 ⇒ H-sum |
| Silver — 2H melee | Percent (%) | 1★ | 1-4 | 1 | 2-5 | 5 ⇒ H-sum |
| Silver — 2H melee | Percent (%) | 2★ | 3-8 | 1-2 | 4-10 | 3 ⇒ H-initial; 1-2 ⇒ H-increment; 9-10 ⇒ H-sum |
| Silver — 2H melee | Percent (%) | 3★ | 5-12 | 2-4 | 7-16 | 5-6 ⇒ H-initial; 2-4 ⇒ H-increment; 13-16 ⇒ H-sum |
| Silver — 2H melee | Percent (%) | 4★ | 9-17 | 3-4 | 12-21 | 9-11 ⇒ H-initial; 3-4 ⇒ H-increment; 18-21 ⇒ H-sum |
| Silver — 2H melee | Percent (%) | 5★ | 12-21 | 3-5 | 15-26 | 12-14 ⇒ H-initial; 3-5 ⇒ H-increment; 22-26 ⇒ H-sum |
| Silver — 2H melee | ASPD | 1★ | 1-2 | 1-2 | 2-4 | 3-4 ⇒ H-sum |
| Silver — 2H melee | ASPD | 2★ | 2-5 | 1-2 | 3-7 | 1 ⇒ H-increment; 6-7 ⇒ H-sum |
| Silver — 2H melee | ASPD | 3★ | 4-8 | 1-4 | 5-12 | 1-3 ⇒ H-increment; 9-12 ⇒ H-sum |
| Silver — 2H melee | ASPD | 4★ | 6-11 | 1-5 | 7-16 | 6 ⇒ H-initial; 1-5 ⇒ H-increment; 12-16 ⇒ H-sum |
| Silver — 2H melee | ASPD | 5★ | 8-13 | 1-7 | 9-20 | 8 ⇒ H-initial; 1-7 ⇒ H-increment; 14-20 ⇒ H-sum |
| Silver — 2H melee | ASPD% | 1★ | 1-2 | 1 | 2-3 | 3 ⇒ H-sum |
| Silver — 2H melee | ASPD% | 2★ | 2-5 | 1-2 | 3-7 | 1 ⇒ H-increment; 6-7 ⇒ H-sum |
| Silver — 2H melee | ASPD% | 3★ | 4-8 | 2-4 | 6-12 | 5 ⇒ H-initial; 2-3 ⇒ H-increment; 9-12 ⇒ H-sum |
| Silver — 2H melee | ASPD% | 4★ | 6-12 | 3-4 | 9-16 | 6-8 ⇒ H-initial; 3-4 ⇒ H-increment; 13-16 ⇒ H-sum |
| Silver — 2H melee | ASPD% | 5★ | 9-14 | 3-5 | 12-19 | 9-11 ⇒ H-initial; 3-5 ⇒ H-increment; 15-19 ⇒ H-sum |
| Silver — 2H melee | SUR | 1★ | 1-2 | 1 | 2-3 | 3 ⇒ H-sum |
| Silver — 2H melee | SUR | 2★ | 2-4 | 1-2 | 3-6 | 1 ⇒ H-increment; 5-6 ⇒ H-sum |
| Silver — 2H melee | SUR | 3★ | 3-5 | 1-2 | 4-7 | 3 ⇒ H-initial; 1-2 ⇒ H-increment; 6-7 ⇒ H-sum |
| Silver — 2H melee | SUR | 4★ | 4-7 | 1-4 | 5-11 | 1-3 ⇒ H-increment; 8-11 ⇒ H-sum |
| Silver — 2H melee | SUR | 5★ | 5-8 | 1-5 | 6-13 | 1-4 ⇒ H-increment; 9-13 ⇒ H-sum |
| Silver — everything else | Flat | 1★ | 2-4 | 1-2 | 3-6 | 1 ⇒ H-increment; 5-6 ⇒ H-sum |
| Silver — everything else | Flat | 2★ | 3-6 | 2-6 | 5-12 | 2 ⇒ H-increment; 7-12 ⇒ H-sum |
| Silver — everything else | Flat | 3★ | 5-8 | 2-10 | 7-18 | 2-4 ⇒ H-increment; 11-18 ⇒ H-sum |
| Silver — everything else | Flat | 4★ | 8-12 | 3-12 | 11-24 | 3-7 ⇒ H-increment; 13-24 ⇒ H-sum |
| Silver — everything else | Flat | 5★ | 11-16 | 3-14 | 14-30 | 3-10 ⇒ H-increment; 17-30 ⇒ H-sum |
| Silver — everything else | Percent (%) | 1★ | 1-4 | 1 | 2-5 | 5 ⇒ H-sum |
| Silver — everything else | Percent (%) | 2★ | 3-7 | 1-2 | 4-9 | 3 ⇒ H-initial; 1-2 ⇒ H-increment; 8-9 ⇒ H-sum |
| Silver — everything else | Percent (%) | 3★ | 5-11 | 2-4 | 7-15 | 5-6 ⇒ H-initial; 2-4 ⇒ H-increment; 12-15 ⇒ H-sum |
| Silver — everything else | Percent (%) | 4★ | 9-16 | 3-4 | 12-20 | 9-11 ⇒ H-initial; 3-4 ⇒ H-increment; 17-20 ⇒ H-sum |
| Silver — everything else | Percent (%) | 5★ | 12-19 | 3-5 | 15-24 | 12-14 ⇒ H-initial; 3-5 ⇒ H-increment; 20-24 ⇒ H-sum |
| Silver — everything else | ASPD | 1★ | 1-2 | 1-2 | 2-4 | 3-4 ⇒ H-sum |
| Silver — everything else | ASPD | 2★ | 2-5 | 1-2 | 3-7 | 1 ⇒ H-increment; 6-7 ⇒ H-sum |
| Silver — everything else | ASPD | 3★ | 4-7 | 1-4 | 5-11 | 1-3 ⇒ H-increment; 8-11 ⇒ H-sum |
| Silver — everything else | ASPD | 4★ | 6-10 | 1-5 | 7-15 | 6 ⇒ H-initial; 1-5 ⇒ H-increment; 11-15 ⇒ H-sum |
| Silver — everything else | ASPD | 5★ | 8-12 | 1-6 | 9-18 | 8 ⇒ H-initial; 1-6 ⇒ H-increment; 13-18 ⇒ H-sum |
| Silver — everything else | ASPD% | 1★ | 1-2 | 1 | 2-3 | 3 ⇒ H-sum |
| Silver — everything else | ASPD% | 2★ | 2-5 | 1-2 | 3-7 | 1 ⇒ H-increment; 6-7 ⇒ H-sum |
| Silver — everything else | ASPD% | 3★ | 4-7 | 2-4 | 6-11 | 5 ⇒ H-initial; 2-3 ⇒ H-increment; 8-11 ⇒ H-sum |
| Silver — everything else | ASPD% | 4★ | 6-11 | 3-4 | 9-15 | 6-8 ⇒ H-initial; 3-4 ⇒ H-increment; 12-15 ⇒ H-sum |
| Silver — everything else | ASPD% | 5★ | 9-13 | 3-5 | 12-18 | 9-11 ⇒ H-initial; 3-5 ⇒ H-increment; 14-18 ⇒ H-sum |
| Silver — everything else | SUR | 1★ | 1-2 | 1 | 2-3 | 3 ⇒ H-sum |
| Silver — everything else | SUR | 2★ | 2-4 | 1-2 | 3-6 | 1 ⇒ H-increment; 5-6 ⇒ H-sum |
| Silver — everything else | SUR | 3★ | 3-5 | 1-2 | 4-7 | 3 ⇒ H-initial; 1-2 ⇒ H-increment; 6-7 ⇒ H-sum |
| Silver — everything else | SUR | 4★ | 4-6 | 1-4 | 5-10 | 1-3 ⇒ H-increment; 7-10 ⇒ H-sum |
| Silver — everything else | SUR | 5★ | 5-7 | 1-5 | 6-12 | 1-4 ⇒ H-increment; 8-12 ⇒ H-sum |
...
4. Derive the increment from the supports, reconvolve to verify, then compare against the guide.
