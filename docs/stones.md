...
**Altering changes what a slot *displays*, not what it rolled.** The game keeps drawing later milestone fills against each slot's *original* blessing, so a piece can end up carrying the same blessing twice — a slot altered to ATK, and a later milestone independently rolling ATK into an empty slot. Players also do it on purpose: when the milestones give you nothing useful, altering a weak slot into a second copy of something good is a real play, low value and all. This is why "which slot was altered" is an input to the Enhancement Oracle rather than bookkeeping — see [`milestone-blessings.md`](./milestone-blessings.md), "Duplicate blessings", for the consequence and the one part of it still open.
